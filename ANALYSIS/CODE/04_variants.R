...
pdf('~/Desktop/variant_plot.pdf', 7, 7)
...
# colorbar(clut = colfunc(5), 
#          x.range = c(length(cities)+2.5, length(cities)+3),
#          y.range = c(1, 4),
...
for(i in 1:4){
  ...
  text(0.02, 20000, c('a)', 'b)', 'c)', 'd)')[i], font = 2)
}
dev.off()
