...
# Date started: 28-07-2021
# Date last modified: 01-09-2021
# Author: Simeon Q. Smeele
# Description: New sourceable version of the analysis of the fundamental frequency traces from Lusicinia. 
# This script:
# - runs DTW (saves the results)
# - runs PCO, PCA and UMAP (saves the results)
# This version is moved to the univariate model folder. 
# This version includes code to output the file and selection names includes in the analysis for comparison. 
# This version works on the new ownCloud folder. 
# This version was moved to the new repo. 
# This version is shortened and cleaned. 
# This version is working on the preprocessed traces. 
# This version uses dist input for umap. 
...
# Loading libraries
libraries = c('data.table', 'tidyverse', 'ape', 'parallel', 'pracma', 'tseries', 'warbleR',
              'dtw', 'umap', 'readxl')
...
path_out = 'ANALYSIS/RESULTS/luscinia/dtw/'
path_log = 'ANALYSIS/RESULTS/luscinia/dtw/dtw_results_log.txt'
path_dtw = paste0(path_out, 'dtw_results.RData')
...
out = mclapply(1:ncol(c), function(x) {
  i = c[1,x]
  j = c[2,x]
  dtw_out = dtw(traces[[i]], traces[[j]])
  return( dtw_out$normalizedDistance )
}, mc.cores = n_cores) %>% unlist # end running through the combinations
out_DTW = list(o = out,
               calls = names(traces),
               key = base_data$key)
write.table(sprintf('DTW is for dataset with key %s.', base_data$key), path_log,
            row.names = F, col.names = F)
save(out_DTW, file = path_dtw) # save output
message('Done.\n')
