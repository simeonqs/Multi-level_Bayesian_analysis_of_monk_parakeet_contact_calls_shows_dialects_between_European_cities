...
# This version runs the model that includes the distance matrix
# source('ANALYSIS/CODE/luscinia/dtw/05_run_dist_models.R')
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
path_out = 'ANALYSIS/RESULTS/luscinia/dtw/models dist/'
...
if(!out_DTW$key == base_data$key) warning('Keys do not match!')
...
for(i in 1:6){
  ...
  dat$d_mat = m_dist_parks[names(trans_parks), names(trans_parks)] # distance in KM
  ...
  # Save
  save(list = c('model', 'dat', 'trans_cities', 'trans_parks'), 
