...
// This version does not include offsets for each level if the call comes from the same. 
data{
...
model{
    ...
    for( n in 1:N_obs ) {
        ...
        (z_park[park_i[n]] * sigma_park + z_park[park_j[n]] * sigma_park) * (1 - same_park[park_pair[n]]) +
        (z_city[city_i[n]] * sigma_city + z_city[city_j[n]] * sigma_city) * (1 - same_city[city_pair[n]]) +
